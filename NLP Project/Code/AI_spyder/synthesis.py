...
class FT():
    def fast_fourier(self, word):
        fs, data = wavfile.read("audio_samples/" + word)
        a = data
        print()
...
class AudioProcess():
    ...
    def audio_split(list_pron, f):
        ...
        for idx, t in enumerate(list_of_timestamps):
            ...
            audio_chunk.export("sounds2/" + list_pron[idx].lower() + ".wav".format(end), format="wav")
...
if __name__ == '__main__':
    ...
    transform = FT()
    files = ["absent.wav"]
    path = "C:\\Users\\DELL\\Downloads\\Projects\\NLP Project\\Code\\AI_spyder\\audio_samples"
...
##        print(f)
    ...
    for word in files:
        print("Audio sample taken or analysis: " + word)
        # ar.record()
        print("")

        list_pron = tts.get_pronunciation(word)
        print("")
        f = AudioProcess.get_duration_wav("audio_samples/" + word)
        ...
        print()
        transform.fast_fourier(word)

        for i in range(len(list_pron)):
            fs, data = wavfile.read("sounds2/"+list_pron[i]+".wav")
            a = data
            print()
